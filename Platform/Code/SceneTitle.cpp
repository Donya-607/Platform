...
void SceneTitle::Draw( float elapsedTime )
{
	...
	auto DrawObjects			= [&]( Target option, bool castShadow )
	{
		...
		if ( Drawable( Target::Boss		) && pBoss		)	{ pBoss->				Draw( &p->renderer ); }
		...
		if ( pFontRenderer )
		{
			...
			auto GetCaption = [&]( Choice item )
			{
				if ( saveDataIsExist )
				{
					switch ( item )
					{
					case Choice::NewGame:	return L"ＮＥＷ ＧＡＭＥ";
					case Choice::LoadGame:	return L"ＬＯＡＤ ＧＡＭＥ";
					default: break;
					}
				}
				else
				{
					switch ( item )
					{
					case Choice::Start:		return L"ＳＴＡＲＴ";
					default: break;
					}
				}
