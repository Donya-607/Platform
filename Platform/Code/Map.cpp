...
bool Tile::ShowImGuiNode( const std::string &nodeCaption )
{
	...
	wantRemove = ImGui::Button( ( nodeCaption + u8"ÇçÌèú" ).c_str() );

	ImGui::Helper::ShowAABBNode( u8"ëÃ",			&body );
	const bool treeIsOpen = StageFormat::ShowImGuiNode( u8"éÌóﬁ", &tileID );
	if ( !treeIsOpen )
	{
		// Show as: "éÌóﬁÅF[%s]"
		ImGui::SameLine();
		ImGui::Text( "ÅF[%s]", StageFormat::MakeIDName( tileID ).c_str() );
	}
